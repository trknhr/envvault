...
Remote STS is a future team-mode architecture and is out of scope for the Local MVP. The Local MVP has no hosted Credlease service, no central profile registry, and no automatic remote trust anchor.
...
The intended UX is that local and remote modes keep the same `credlease://` reference shape while changing the issuer backend and trust anchor.
