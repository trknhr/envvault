...
EnvVaultは、長期credentialを`.env`や対象プロセスへ置かず、実行時に用途・resource・scope・TTLを限定した短命credentialへ交換して貸し出す、ローカルファーストなcredential launcherである。

AI agentは代表的な利用者だが、EnvVaultはagent専用ではない。`codex`、開発サーバー、テスト、migration、CLI、ブラウザログインなど、ローカルで起動する任意の開発フローを対象とする。
...
Local MVPでは、エンドユーザーが明示的にインストールするものはGo製の`envvault`単一バイナリだけとする。EnvVaultはOry Talosをmanaged runtimeとして必要時だけ起動し、Talos親APIキーから短命JWTを派生させる。

長期のraw secretはOS Credential Storeへ保存する。Talos SQLiteにはhashとmetadataのみを保存し、Repositoryおよび対象プロセスにはraw parent keyを渡さない。

## 2. プロダクトの定義

> EnvVaultは、プロジェクト内のcredential参照を、実行時に短命・最小権限のcredentialへ解決し、対象プロセスまたはブラウザセッションへ一時的に貸し出すOSSである。

EnvVaultはSecret Managerそのものではない。保存済みの長期secretをそのまま注入するのではなく、信頼anchorを使って実行ごとに新しいcredentialを発行する。
...
EnvVault Local MVPは次のレベルを提供する。

| Level | 内容 | EnvVault Local MVP |
|---|---|---:|
...
- `.env`内の`envvault://<profile>`参照を短命JWTへ解決する。
- `.env`内の`envvault://<profile>/base-url`と`envvault://<profile>/token`参照を3P API向けlocalhost proxyへ解決する。
- Profileごとにresource、scope、TTLを固定し、Repository側から権限を拡大できないようにする。
- 3P API proxy profileごとにtarget URL、method/path allowlist、local token TTLを固定する。
- Ory Talos OSSを利用者に意識させず、SQLite・single-nodeで自動管理する。
- 短命JWTのraw値をdiskへ永続化しない。
- JWT検証用JWKSをstableなfileとして提供する。
- `envvault open`により、1P Webアプリのbrowser sessionをID/パスワードなしで開始できる。
...
- Codexが`envvault token`を再実行することの完全防止。
- prompt、stdout、stderr、HTTP bodyのDLPや自動redaction。
- VM、container、sandbox、egress proxyの提供。
- custom base URLを設定できないSDKやtoolから、3P provider keyを隠すこと。
...
envvault open admin-web/dev
        ...
        ▼
Backend POST /auth/envvault/browser-sessions
...
envvault
        ...
        ▼
Browser GET /auth/envvault/complete?code=<opaque>
...
Talosは汎用API key serverでありagent専用ではない。EnvVaultでは発行エンジンとして利用する。
...
├─ envvault/talos/signing/<kid>
└─ envvault/profile/<profile-id>/parent-key
...
- Browser login codeをEnvVault側で保存しない。
...
profiles:
  ...
  admin-web/dev:
    ...
    login_code_ttl: 30s
    web_session_ttl: 30m
    exchange_url: https://admin.dev.example.com/auth/envvault/browser-sessions
    complete_url: https://admin.dev.example.com/auth/envvault/complete
...
- Profileはユーザー側のEnvVault configへ保存し、Repository内設定をauthorityにしない。
...
ただし`envvault://`参照がある変数について、親環境の同名secretへfallbackしてはならない。
...
2. EnvVault release manifestから互換Talos versionを選択。
...
```bash
envvault token backend-a/dev
envvault token backend-a/dev --format json
```
...
```http
POST /auth/envvault/browser-sessions HTTP/1.1
...
{
  "launch_url": "https://admin.dev.example.com/auth/envvault/complete?code=<opaque>",
...
- EnvVault releaseごとに互換Talos versionをpinする。
...
{
  "name": "envvault:backend-a/dev",
  "actor_id": "envvault-local:<installation-id>",
...
- Derive前にEnvVault側でもrequested scopeがProfile scopeのsubsetであることを検証する。
- Talosが拒否したscope、TTL、inactive parentを上書きやretryで緩和しない。
- Reserved standard claimsを`custom_claims`へ指定しない。EnvVault custom claimは`envvault_` prefixを使用する。
- JWKSは`GET /v2alpha1/derivedKeys/jwks.json`から取得し、public fileへexportする。
- `v2alpha1`の変更はadapter testで検出し、EnvVault release manifestの互換versionを更新するまで新versionを使用しない。
...
```json
{
  ...
  "envvault_resource": "https://api.dev.example.com",
  "envvault_session_id": "01J...",
  "envvault_project_id": "sha256:...",
  "envvault_user": "local-user",
...
Backendは`envvault_resource`を必須検証する。Talos adapterが標準`aud`をprofile単位で安全に設定できるようになった場合は、`aud`も必須検証へ移行する。
...
- Remote 1P Backendへ当該EnvVault installationのpublic JWKSが事前登録されている。

任意のローカルinstallationが生成した鍵をRemote Backendが自動的に信用してはならない。組織共通のRemote Backendは将来の`envvault-sts`を推奨する。
...
3. `envvault_session_id`をreplay cacheへatomic insertする。
...
- `post_login_url`はserver configとEnvVault Profileの両方で固定する。
...
```text
envvault open legacy-console
  ↓ one-time fill lease
Chrome Extension
  ↓ Native Messaging
EnvVault Native Host
...
- Native Messaging Hostは`envvault native-host`。
...
| ID | 要件 |
|---|---|
| FR-001 | 単一の`envvault`バイナリで初期化できる |
...
| FR-014 | `envvault open`でbrowser bootstrap sessionを開始できる |
...
- `envvault open`のbrowser launch開始: p95 4秒以内。
- EnvVault自身の追加RSS: 50MB未満を目標。
...
| Code | 意味 |
|---|---|
| `ENVVAULT_CONFIG_INVALID` | 設定不正 |
| `ENVVAULT_PROFILE_NOT_FOUND` | 未知のProfile |
| `ENVVAULT_PROFILE_KIND_MISMATCH` | commandとProfile kindが不一致 |
| `ENVVAULT_PROJECT_NOT_TRUSTED` | project binding未承認 |
| `ENVVAULT_KEYRING_UNAVAILABLE` | OS Credential Store利用不可 |
| `ENVVAULT_KEYRING_LOCKED` | Credential Storeがlocked |
| `ENVVAULT_PARENT_KEY_MISSING` | parent key欠落 |
| `ENVVAULT_RUNTIME_UNAVAILABLE` | Talos起動失敗 |
| `ENVVAULT_RUNTIME_INCOMPATIBLE` | Talos version不一致 |
| `ENVVAULT_ISSUE_FAILED` | JWT derive失敗 |
| `ENVVAULT_REFERENCE_INVALID` | Reference構文不正 |
| `ENVVAULT_BROWSER_EXCHANGE_FAILED` | Browser exchange失敗 |
| `ENVVAULT_BROWSER_URL_REJECTED` | launch URLがpolicy不一致 |
| `ENVVAULT_CLEANUP_FAILED` | cleanup失敗 |
| `ENVVAULT_LOCK_TIMEOUT` | runtime lock取得timeout |
...
| 同一ユーザーprocessが`envvault token`を実行 | 完全防止しない |

### 22.3 残存リスク

- Leased tokenはBearerであり、有効中はコピー利用できる。
- 同一OSユーザーのprocessはEnvVault CLIを起動できる。
...
**Then** EnvVault config/data/cache、SQLite、logs、audit、temporary directoryをbinary-safe scanしてもcanaryが存在しない。
...
**Given** `.env`に`TOKEN=envvault://backend-a/dev`がある
**When** `envvault exec --env-file .env -- inspect-child`を実行する
...
**When** `envvault exec`を実行する
**Then** childは起動されない
**And** `ENVVAULT_PROFILE_NOT_FOUND`を返す
**And** 親環境の同名secretへfallbackしない。

### AT-REF-003: Queryによる権限昇格拒否

**Given** `TOKEN=envvault://backend-a/dev?scope=admin&ttl=24h`
**When** `envvault exec`を実行する
**Then** childは起動されない
**And** `ENVVAULT_REFERENCE_INVALID`を返す。

### AT-EXEC-001: Childへparent authorityを渡さない

**Given** 有効なprocess Profile
**When** `inspect-child`を`envvault exec`で起動する
...
**When** `envvault exec`で`inspect-child`を起動する
...
**When** `envvault exec -- <command>`を実行する
**Then** EnvVaultは42を返す。

### AT-EXEC-004: Signal伝播

**Given** 長時間動作するchild
**When** EnvVaultへSIGINTを送る
**Then** childへSIGINTが伝播する
**And** EnvVaultはplatform標準の終了semanticを保持する。
...
**Given** `envvault jwks export`で生成したJWKS
...
**Given** Talos起動中にEnvVault processを強制終了する
**When** 次回`envvault doctor --repair`を実行する
**Then** stale process、lock、temporary secret fileをcleanupする
**And** DB integrityを確認する。

### AT-CONCURRENCY-001: 同時発行

**Given** 同一installationで10並列の`envvault token`
**When** 同時実行する
**Then** deadlockしない
...
**When** `envvault open admin-web/dev`を実行する
...
**Given** 同一`envvault_session_id`を持つbootstrap JWT
...
**When** `envvault open`を実行する
**Then** browserを起動せず
**And** `ENVVAULT_BROWSER_URL_REJECTED`を返す。
...
**When** `envvault init`またはtoken issueを実行する
**Then** 平文file fallbackを作らず
**And** `ENVVAULT_KEYRING_UNAVAILABLE`を返す。

### AT-RESET-001: Reset

**Given** 初期化済みinstallation
**When** `envvault reset`を明示確認付きで実行する
...
- `envvault credential-process <profile>`。
...
- CI OIDC federation。
- local/remote modeで同じ`envvault://` UX。
...
> EnvVaultは、長期credentialを`.env`や対象processへ置かず、ローカルの信頼anchorから実行単位の短命credentialまたはbrowser sessionを発行して貸し出すOSSである。
