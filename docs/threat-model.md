...
- Child process environment: receives short-lived leased tokens, local proxy bearer tokens, or raw inject values when explicitly configured.
...
- Tools that require raw credentials and are configured through explicit inject profiles.
...
- Inject profiles require an explicit `/value` reference and project binding before a raw credential is passed to a child process.
...
improves repository and `.env` hygiene but does not hide that credential from
